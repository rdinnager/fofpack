...
knitr::include_graphics("images/week_9_lecture_insertimage_1.png")
```
...
autoplot(data_splits)
autoplot(data_splits$splits[[1]])
autoplot(data_splits$splits[[2]])
```
...
```{r do_model, exercise = TRUE}
RF_recipe <- recipe(training(data_split$splits[[1]]),
