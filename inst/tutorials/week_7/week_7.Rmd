...
### Save the tuning object and the model
...
save_to <- file.path(getwd(), "workflow_and_tuning.rds")
saveRDS(list(workflow = PR_wf, tuning = PR_tune),
        save_to)
cat(save_to)
```
...
In `fofpack` there is a function called `plot_marginal_predictions()`

Use the R help system to find out what it's arguments are, then try and use it to visualise the model you have just fit.
