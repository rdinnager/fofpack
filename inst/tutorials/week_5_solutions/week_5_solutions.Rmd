---
title: "Forests of the Future: Week 5 Assignment with Solutions"
output: 
  learnr::tutorial:
    progressive: false
    language:
...
description: Forests of the Future Week 5 Assignment with Solutions
---
...
mysp <- "Gnathodentex aureolineatus"
```
...
```{r e2_2-solution}
ggplot(mysp_dat, aes(x = MeanTemp_CoralWatch, y = log_Abundance)) + 
  geom_point() +
  theme_minimal()
```
...
```{r e3-solution}
form <- log_Abundance ~ MeanTemp_CoralWatch
...
```{r e3_2-solution}
mysp_preds <- predict(mysp_mod, mysp_dat)
```
...
```{r b4-solution}
pred_dat_2 <- augment(mysp_mod_2, pred_dat_2)

ggplot(pred_dat_2, aes(MeanTemp_CoralWatch, Depth_Site)) +
  geom_contour_filled(aes(z = .pred)) +
