...
description: Forests of the Future Week 5 Lecture 
---
...
preds <- predict(mod_pars, newdata = fish_dat)
preds
pred_dat <- fish_dat %>%
  bind_cols(preds)
```
...
preds2 <- predict(mod_pars2, newdata = fish_dat)
pred_dat2 <- fish_dat %>%
  bind_cols(preds2) 
```
...
- The [FIU Preserve](https://sustainability.fiu.edu/the-fiu-nature-preserve/) on campus has one of the few remaining patches of Pine Rockland in Miami-Dade
- In 2016 FIU conducted a prescribed burn to help restore this ecosystem of local importance

<iframe src="https://www.youtube.com/embed/YhwmIwczW6c" data-external= "1"> </iframe>
