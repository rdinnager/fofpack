...
- Outputs are predictions, and they are compare to 'response' variable(s)
...
```{r, out.width = "300px"}
knitr::include_graphics("images/week_5_lecture_insertimage_1.png")
```
...
```{r RF3, exercise=TRUE}
mod <- lm(AbundanceAdult40 ~ MeanTemp_CoralWatch,
          data = fish_dat)
```
...
```{r abund_dist2,echo=TRUE}
 hist(log(fish_dat$AbundanceAdult40), breaks = 100)
```
...
- `parsnip` is designed work with pipe operatoros (`%>%` or `|>`) 
...
preds <- predict(mod_pars, fish_dat)
preds
pred_dat <- fish_dat %>%
  bind_cols(preds) %>%
  arrange(MeanTemp_CoralWatch)
```
...
p <- ggplot(pred_dat, aes(x = MeanTemp_CoralWatch, y = .pred)) +
  geom_path() + geom_point(aes(y = AbundanceAdult40)) + 
  theme_minimal()
suppressMessages(print(p))
```
...
preds2 <- predict(mod_pars2, fish_dat)
pred_dat2 <- fish_dat %>%
  bind_cols(preds2) %>%
  arrange(MeanTemp_CoralWatch)
```
...
- Why?
  - Pine Rocklands central range is in the middle of 